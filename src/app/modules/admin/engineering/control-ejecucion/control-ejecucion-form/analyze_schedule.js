    ...
    for (let r = 17; r <= 35; r++) {
        ...
        for (let c = 6; c <= 40; c++) {
            ...
            if (cell.fill && cell.fill.type === 'pattern' && cell.fill.pattern !== 'none') {
                filledCols.push({ col: c, fill: cell.fill });
            } else if (cell.value !== null && cell.value !== '') {
                filledCols.push({ col: c, val: cell.value });
            }
        }
        if (filledCols.length > 0) {
            console.log(`Row ${r} (${name.substring(0, 30)}):`, filledCols.map(f => `Col${f.col}: [${f.val !== undefined ? JSON.stringify(f.val) : 'Filled'}]`).join(' | '));
        }
    }
}
